...
### 4.6 본문 섹션과 자료 연결 — `::section[...]`

보조자료를 **본문의 어느 구간에서 보여줄지** 정합니다. 마커를 쓰면 우측 패널이
**"지금 읽는 구간의 자료"를 자동으로 보여주는 따라오기 모드**로 동작합니다.

```markdown
::section[fig-venn]        ← 섹션 1 시작. 자료 fig-venn 과 연결

## 집합이란
본문 1

::section[note-symbols]    ← 섹션 2 시작. 여기서 섹션 1이 끝난다
## 원소와 기호
본문 2

::section[fig-venn]        ← 같은 자료를 다시 호출해도 된다(중복 허용)
## 다시 원소
본문 3

::section[]                ← 빈 섹션. 자료는 없지만 경계 역할은 한다
## 잠깐 쉬어가기
본문 4
```
...
| 항목 | 내용 |
|---|---|
| 마커의 의미 | **섹션의 시작**. 다음 마커 직전까지가 그 섹션 |
| 위치 | `::section[...]` 를 **단독 줄**에 쓴다 |
| 빈 섹션 | `::section[]` — 연결할 자료가 없음 |
| 중복 호출 | 같은 id 를 여러 섹션에서 불러도 된다 |
| 없는 id | `::section[없는id]` → 빈 섹션으로 처리(경고만, 빌드 성공) |
| 마커가 없으면 | **전체 목록 모드**로 폴백(기존 동작) |
| 첫 마커 이전 | 첫 섹션으로 간주 |

**패널 동작**

- `따라오기`(기본): 읽는 구간이 바뀌면 그 구간의 자료로 교체됩니다. 자료가 화면 밖으로 나가지 않습니다
- `전체 목록`: `materials` 에 적은 자료를 전부 위아래로 훑어볼 수 있습니다
- 빈 섹션을 지나면 **이전 자료를 유지**하고 `이 구간에 연결된 자료가 없습니다` 배지가 뜹니다
- 본문의 섹션 마커에 마우스를 올리면 해당 자료 카드가, 자료 카드에 올리면 본문 마커가 강조됩니다

**인쇄**

- 인쇄 부록은 `materials:` 에 적힌 자료를 **순서대로 1회씩만** 출력합니다.
  본문에서 여러 번 호출해도 인쇄에는 영향이 없습니다.

> 마커를 쓰지 않아도 글은 정상입니다(전체 목록 모드). 다만 **따라오기**를 쓰려면 마커가 필요합니다.

### 4.7 기존 방식 — `<FigureRef>`

`<FigureRef id="fig-venn" n={1}>그림으로 보기</FigureRef>` 는 **문장 안에서 특정 자료를 가리키는** 링크입니다.
클릭하면 패널이 열리고 그 자료로 이동합니다. 섹션 마커와 함께 써도 됩니다.

- `::section[...]` = "이 구간은 이 자료" (구간 지정, 따라오기)
- `<FigureRef>` = "이 문장에서 저 자료를 보라" (지점 지정, 점프)
...
- [ ] `::section[...]` 마커가 가리키는 id가 `materials`에 있는가 (없으면 `R16` 경고)
- [ ] 호출되지 않은 자료가 있는가 (있으면 `R17` 경고 — 인쇄에는 포함됨)
- [ ] `links.requires`가 실제로 존재하는 글인가
- [ ] `npm run validate`가 **오류 0**인가
- [ ] `npm run dev`에서 좌측 네비·우측 패널·이전/다음 글이 정상인가
- [ ] 우측 패널 `따라오기`가 스크롤에 따라 자료를 바꾸는가
