...
M2까지 완료: 본문 라우트·헤더/브레드크럼·좌측 네비 트리·패널 2탭·자료 카드·본문↔패널 이동·목차 scrollspy·이전/다음 글.

### 5.1 패널 폭 조절 (핵심) — ✅ 완료

- [x] 드래그 리사이즈 핸들: `role="separator"`, `aria-orientation="vertical"`, `tabindex="0"`, `aria-controls`, `aria-valuemin/max/now`
- [x] 폭 범위 `320~560px`, 기본 `380px`(≥1440) / `320px`(1024~1439)
- [x] 리사이즈 결과를 `--panel-current`에 반영
- [x] `localStorage` 저장(`panel.width`) + **리셋 버튼**(패널 헤더)
- [x] 창이 좁아지면 **컨테이너의 45%** 로 클램프
- [x] 드래그 중 `data-state="dragging"` + 핸들 시각 표시
- [x] 키보드: `←` 넓게 / `→` 좁게 16px, `Shift` 64px, `Home/End` 최소/최대, `Enter` 접기·펼치기
- [x] 폭 전환은 그리드 컬럼 트랜지션(리플로우 최소화)
...
- [x] 자료 이미지 클릭 → 확대 보기(원본 보기·다운로드·`n / m` 표시)
- [x] 키보드: `Esc` 닫기, `←/→` 이전·다음, 포커스 트랩, 닫은 뒤 원래 요소로 포커스 복귀
- [x] `aria-modal="true"`, 배경 클릭으로 닫기, 배경 스크롤 락

### 5.4 모바일(사이트 쪽) — 🔶 부분 완료

- [x] 바텀 시트: **드래그로 높이 조절**(화면의 25~90%)
- [ ] 스냅 지점(`25% / 50% / 90%`)으로 자동 정착 — 미구현
- [x] 스크림 탭 / `Esc` 닫기, 배경 스크롤 락
- [ ] 드로어·시트 **포커스 트랩** — 미구현
- [x] `<1024px`에서 좌 nav와 우 패널 동시 열기 금지

### 5.5 인쇄 — ✅ 완료

- [x] `print.css`: 헤더·네비·패널 크롬 제거, 본문 전폭
- [x] **패널 자료를 본문 뒤 부록으로 펼쳐 출력**(`부록: 보조 자료`, 페이지 나눔)
- [x] 수식·코드·표·그림 `break-inside: avoid`, 외부 링크 URL 각주

### 5.6 검증

- [ ] 6장 수용 기준 10항목 전부 통과 (8번 포커스 트랩이 미구현)
- [x] `npm run check` 통과(오류 0)
- [ ] 4개 브레이크포인트 육안 확인 — **사용자 확인 필요** (브라우저 도구 없음)
