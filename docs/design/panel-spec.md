...
M2까지 완료된 것: 본문 라우트·헤더/브레드크럼·좌측 네비 트리·패널 2탭·자료 카드·본문↔패널 이동·목차 scrollspy·이전/다음 글.
**아래가 M3에서 남은 작업이다.**

### 5.1 패널 폭 조절 (핵심)

- [ ] 드래그 리사이즈 핸들: `role="separator"`, `aria-orientation="vertical"`, `tabindex="0"`, `aria-controls`, `aria-valuemin/max/now`
- [ ] 폭 범위 `320~560px`, 기본 `380px`(≥1440) / `320px`(1024~1439)
- [ ] 리사이즈 결과를 `--panel-current`에 반영(AppShell이 이미 이 변수를 읽도록 되어 있음)
- [ ] `localStorage` 저장(`panel.width`) + **리셋 버튼**
- [ ] 창이 좁아지면 `min(사용자값, 컨테이너의 45%)`로 클램프
- [ ] 드래그 중 `data-state="dragging"` 부여
- [ ] 키보드: `←/→` 16px, `Shift+←/→` 64px, `Home/End` 최소/최대, `Enter` 접기/펼치기
- [ ] 폭 전환은 `grid-template-columns` 트랜지션(리플로우 최소화)

### 5.2 접힘/펼침 완성도

- [ ] `200ms ease-out`, `prefers-reduced-motion`에서 즉시 전환
- [ ] **스크롤 앵커**: 폭이 바뀌어도 읽던 문단 위치 유지
- [ ] 접힘 상태에서 우측 모서리에 **세로 탭(폭 28px)** 으로 자료 존재 인지
- [ ] `panel.open` / `panel.width` / `panel.tab` / `panel.group` 4개 상태 저장·복원

### 5.3 라이트박스

- [ ] 자료 이미지 클릭 → 확대 보기(원본 보기·다운로드)
- [ ] 키보드: `Esc` 닫기, `←/→` 이전/다음, 포커스 트랩, 닫은 뒤 원래 요소로 포커스 복귀
- [ ] `aria-modal="true"`, 배경 스크림

### 5.4 모바일(사이트 쪽) 완성

- [ ] 바텀 시트: 스냅 지점 `25% / 50% / 90%`, 드래그로 높이 조절
- [ ] 스크림 탭 / `Esc` 닫기, 포커스 트랩, 배경 스크롤 락
- [ ] `<1024px`에서 좌 nav와 우 패널 동시 열기 금지(구현됨 — 회귀 테스트만)

### 5.5 인쇄

- [ ] `print.css`: 좌 nav·헤더·패널 크롬 제거, 본문 전폭
- [ ] **패널 자료를 본문 뒤 부록으로 펼쳐 출력**
- [ ] 수식·코드가 잘리지 않게 처리

### 5.6 검증

- [ ] 아래 6장의 **수용 기준 10항목** 전부 통과
- [ ] `npm run check` 통과(오류 0)
- [ ] 4개 브레이크포인트 육안 확인(브라우저 도구가 없으면 사용자 확인)
